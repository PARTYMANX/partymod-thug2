...
void defaultSettings() {
	...
	keybinds.focus = SDL_SCANCODE_KP_ENTER;
	keybinds.pause = SDL_SCANCODE_ESCAPE;

	keybinds.item_up = SDL_SCANCODE_I;
	keybinds.item_down = SDL_SCANCODE_K;
	keybinds.item_left = SDL_SCANCODE_J;
	keybinds.item_right = SDL_SCANCODE_L;
	...
	keybinds.camUp = SDL_SCANCODE_KP_PLUS;
	keybinds.camDown = SDL_SCANCODE_KP_MINUS;
	keybinds.camLeft = SDL_SCANCODE_KP_DIVIDE;
	keybinds.camRight = SDL_SCANCODE_KP_MULTIPLY;
	keybinds.viewToggle = SDL_SCANCODE_F;
	...
	padbinds.focus = CONTROLLER_BUTTON_LEFTSTICK;
...
void loadSettings() {
	...
	keybinds.focus = GetPrivateProfileInt("Keybinds", "Focus", SDL_SCANCODE_KP_ENTER, configFile);
	keybinds.pause = GetPrivateProfileInt("Keybinds", "Pause", SDL_SCANCODE_RETURN, configFile);

	keybinds.item_up = GetPrivateProfileInt("Keybinds", "ItemUp", SDL_SCANCODE_I, configFile);
	keybinds.item_down = GetPrivateProfileInt("Keybinds", "ItemDown", SDL_SCANCODE_K, configFile);
	keybinds.item_left = GetPrivateProfileInt("Keybinds", "ItemLeft", SDL_SCANCODE_J, configFile);
	keybinds.item_right = GetPrivateProfileInt("Keybinds", "ItemRight", SDL_SCANCODE_L, configFile);
	...
	keybinds.viewToggle = GetPrivateProfileInt("Keybinds", "ViewToggle", SDL_SCANCODE_F, configFile);
...
void saveSettings() {
	...
	writeIniBool("Graphics", "DistanceClipping", settings.distance_clipping, configFile);
	writeIniInt("Graphics", "ClippingDistance", settings.clipping_distance, configFile);
	...
	writeIniInt("Keybinds", "Focus", keybinds.focus, configFile);
	...
	writeIniInt("Keybinds", "Feeble", keybinds.feeble, configFile);
	...
	writeIniInt("Gamepad", "Focus", padbinds.focus, configFile);
...
void build_gamepad_page(pgui_control *parent) {
	...
	pgui_label_create(8, 16 + label_offset + (graphics_v_spacing * 8), 96, 16, "Focus:", PGUI_LABEL_JUSTIFY_LEFT, actions_groupbox);
	gamepad_page.focus = build_button_combobox(actions_groupbox->w - 8 - box_width, 16 + (graphics_v_spacing * 8), box_width, 20, actions_groupbox, &(padbinds.focus));
...
void setAllPadBindText() {
	...
	setButtonBindBox(gamepad_page.focus, padbinds.focus);
...
void build_keyboard_page(pgui_control *parent) {
	...
	pgui_label_create(8, 16 + label_offset + (graphics_v_spacing * 8), 96, 16, "Focus:", PGUI_LABEL_JUSTIFY_LEFT, actions_groupbox);
	keyboard_page.focus = build_keybind_textbox(actions_groupbox->w - 8 - 64, 16 + (graphics_v_spacing * 8), 64, 20, actions_groupbox, "Focus", &(keybinds.focus));

	pgui_label_create(8, 16 + label_offset + (graphics_v_spacing * 9), 96, 16, "Pause:", PGUI_LABEL_JUSTIFY_LEFT, actions_groupbox);
	keyboard_page.pause = build_keybind_textbox(actions_groupbox->w - 8 - 64, 16 + (graphics_v_spacing * 9), 64, 20, actions_groupbox, "Pause", &(keybinds.pause));
...
void setAllBindText() {
	...
	setBindText(keyboard_page.focus, keybinds.focus);
...
void build_general_page(pgui_control *parent) {
	...
	general_page.clipping_distance_far = pgui_label_create(160 - 32 + 4, 16 + (24 * 4) + 4, 32, 24, "Far", PGUI_LABEL_JUSTIFY_RIGHT, graphics_groupbox);
	...
	general_page.prefer_glyphs = pgui_checkbox_create(8, 16 + (24 * 2), 128, 24, "Prefer Controller Glyphs", misc_groupbox);
	general_page.disable_blur = pgui_checkbox_create(8, 16 + (24 * 3), 128, 24, "Disable Blur Effect*", misc_groupbox);
	...
	pgui_checkbox_set_on_toggle(general_page.prefer_glyphs, do_setting_checkbox, &(settings.prefer_glyphs));
...
void update_general_page() {
	...
	pgui_checkbox_set_checked(general_page.prefer_glyphs, settings.prefer_glyphs);
